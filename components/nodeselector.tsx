            ...
            <SheetContent side="right" className="w-full sm:max-w-md">
                ...
                <div className="grid gap-4 py-4">
                    {triggerNodes.map((node) => (
                        <div key={node.type} className="w-full justify-start h-auto py-5 px-4 rounded-none cursor-pointer border-l-2 border-transparent hover:border-l-primary" onClick={()=> handleNodeSelect(node)}>
                            <div className="flex items-center gap-6 w-full overflow-hidden">
                                {typeof node.icon === "string" ? (
                                    <Image src={node.icon} alt={node.label} className="size-5 object-contain rounded-sm" />
                                ) : (
